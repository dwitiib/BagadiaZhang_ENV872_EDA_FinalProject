...
NY_SO2_clean = NY_SO2_clean %>% filter(Sample.Duration == "1 HOUR") %>% group_by(County.Name) %>% summarise(meanSO2 = mean(AQI))
...
NY_CO_clean = NY_CO_clean %>% filter(Sample.Duration == "8-HR RUN AVG END HOUR") %>% group_by(County.Name) %>% summarise(meanCO = mean(AQI))
...
NY_NO2_clean = NY_NO2_clean %>% group_by(County.Name) %>% summarise(meanNO2 = mean(AQI))
...
NY_PM25_clean = NY_PM25_clean %>% filter(Sample.Duration == "24-HR BLK AVG") %>% group_by(County.Name) %>% summarise(meanPM25 = mean(AQI))
...
NY_PM10_clean = NY_PM10_clean %>% group_by(County.Name) %>% summarise(meanPM10 = mean(AQI))
colnames(NY_Ozone_clean) = c("County", "meanpm10")
